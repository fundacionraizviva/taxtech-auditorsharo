...
<style>
    ...
    .tabla-contable .titulo-anio { text-align: center; font-weight: bold; font-size: 1rem; background-color: #f1f5f9; padding: 8px !important; border-bottom: 2px solid #000;}
</style>
...
# ──────────────────────────────────────────────────────────────────────────────
# PROCESAMIENTO DE BALANZA (ESTABLE - SIN CACHÉ)
# ──────────────────────────────────────────────────────────────────────────────
...
if uploaded is None: st.info("👆 Sube una balanza para iniciar."); st.stop()
...
tab_comp, tab_bg, tab_er, tab_efe, tab_ppe, tab_bal, tab_inconsist, tab_art287, tab_ir2, tab_consol = st.tabs([
    ...
    "📋 Balanza Creada", "🚨 Inconsistencias", "⚖️ Riesgos Art.287", "📝 IR-2", "🏛️ Consolidado Fiscal"
])
...
with tab_comp:
    ...
    with c2:
        ...
        if excel_bytes:
            st.download_button("📥 Descargar Excel Corporativo", data=excel_bytes, file_name=f"Reporte_{empresa}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_ir2:
    st.markdown("### 📝 Borrador IR-2")
    st.json(ir2_vals)

with tab_consol:
    isr_est = max(0, utilidad_neta) * 0.27
    df_consol = pd.DataFrame([
        ("IR-2 / ISR", "Impuesto sobre la Renta (27%)", f"RD$ {isr_est:,.2f}", "Proyección Estimada", "⚠️" if isr_est > 0 else "✅")
