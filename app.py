...
# ==========================================
# 1. PARAMETRIZACIÓN FISCAL DOMINICANA (2026)
# ==========================================
NATURALEZAS = {
...
# Estilos de Diseño Corporativo para Excel
...
def escribir_datos_excel(ws, start_row, dataset, total_label="TOTAL", sum_column_indices=[]):
    ...
    for idx, fila in enumerate(dataset):
        for c_idx, valor in enumerate(fila, start=2):
            ...
            if isinstance(valor, (float, int)):
                if valor <= 1.0 and c_idx == 4:
                    cell.number_format = '0.00%'; cell.alignment = Alignment(horizontal="center")
                else:
                    cell.number_format = 'RD$ #,##0.00'; cell.alignment = Alignment(horizontal="right")
    ...
    for c in range(2, len(ws[5]) + 2):
        ...
        cell.font = FONT_BOLD; cell.fill = FILL_TOTAL; cell.border = CELL_BORDER
...
# ==========================================
# 4. ENTORNO INTERACTIVO DE USUARIO
# ==========================================
...
if uploaded_file is not None:
    ...
    if not df_balanza.empty:
        ...
        with tab2:
            ...
            if not df_err.empty:
                st.error("Se detectaron inconsistencias en la naturaleza de los saldos.")
        ...
        with tab5:
            ...
            df_it1_pantalla = pd.DataFrame({
                ...
                'Monto Control Contable': [monto_ingresos_gravados, itbis_ventas_generado, itbis_soportado_compras, itbis_ret_100_fisicas, itbis_ret_30_juridicas, itbis_ret_tarjetas_2]
            })
            st.dataframe(df_it1_pantalla.style.format({'Monto Control Contable': 'RD$ {:,.2f}'}), use_container_width=True, hide_index=True)
        ...
        with tab6:
            ...
            df_tss_pantalla = pd.DataFrame({
                ...
                'Sueldo Cotizable': [gasto_nominas_global * 0.40, gasto_nominas_global * 0.35, gasto_nominas_global * 0.25],
                'Dependientes Adicionales': [num_dependientes_estimados if num_dependientes_estimados > 0 else 0, 0, 0]
            })
            df_tss_pantalla['SFS Empleado (3.04%)'] = df_tss_pantalla['Sueldo Cotizable'] * TASA_SFS_EMPLEADO
        ...
        with tab7:
            ...
            df_ir17_pantalla = pd.DataFrame({
                ...
                'Concepto Oficial DGII': [
                    ...
                    'Retribuciones Complementarias - Vehículos / Combustible', 'Retribuciones Complementarias - Pago de Alquileres / Renta',
                    'Retribuciones Complementarias - Otros Beneficios en Especie', 'Remesas al Exterior - CDI España', 
                    'Remesas al Exterior - CDI Canadá', 'Remesas al Exterior - Otros Países (General)'
                ],
                'Tasa Aplicada': ['10%', '2%', '27%', '27%', '27%', '10%', '18%', '27%'],
                'Monto Base Imponible': [b_honorarios, b_reparaciones, b_vehiculos, b_renta_vivienda, b_otras_retribuciones, b_espana, b_canada, b_exterior_general],
                'Impuesto Determinado': [r_honorarios, r_reparaciones, r_vehiculos, r_renta, r_otras_ret, r_espana, r_canada, r_exterior]
            })
            st.dataframe(df_ir17_pantalla.style.format({'Monto Base Imponible': 'RD$ {:,.2f}', 'Impuesto Determinado': 'RD$ {:,.2f}'}), use_container_width=True, hide_index=True)
